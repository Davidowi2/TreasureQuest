...
import { Copy, Users, Play, X, Shield, ArrowLeft } from "lucide-react";
...
export default function TeamLobby() {
  ...
  const canStart = isLeader && activeMembersCount >= 2;
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-2xl min-h-[calc(100vh-4rem)] flex flex-col">
      ...
      <Card className="border-2 shadow-xl bg-card relative overflow-hidden mb-8">
      ...
      <div className="mt-8">
        {isLeader ? (
          ...
          >
            {canStart ? (
              <><Play className="mr-2" /> Start Adventure</>
            ) : (
              "Waiting for at least 1 other active player..."
            )}
          </Button>
        ) : (
